...
int EnemyComponent::GetMovingDirection(GameContext& context)
{
		...
		(ey - searchArea <= ty) && (ey + searchArea >= ty))
	{
		...
		generator.SetDiagonalMovement(false);	// ç°âÒÇÕéŒÇﬂà⁄ìÆÇµÇ»Ç¢ÇÃÇ≈ false

		// ï«Ç Astar ÉAÉãÉSÉäÉYÉÄÇ÷ìoò^Ç∑ÇÈ
		...
		std::vector<DirectX::SimpleMath::Vector2> data;	// ÉãÅ[ÉgèÓïÒ
		{
